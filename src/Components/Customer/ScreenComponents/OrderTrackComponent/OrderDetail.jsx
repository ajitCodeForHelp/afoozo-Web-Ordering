...
const OrderDetails = ({ OrderDetail }) => {
  return (
    <>
      {/* <div className="my-3 shadow-sm px-3 fs-6 fw-semibold pb-3">
        Bill Details
      </div> */}
      <div className="container my-4">
        <div className="card shadow-sm p-3 rounded-4">
          ...
          {OrderDetail?.couponDiscountAmount > 0 && <div className="d-flex justify-content-between">
            <span className="text-muted">Coupon Discount</span>
            <span className="fw-medium">₹{Number(OrderDetail?.couponDiscountAmount).toFixed(2)}</span>
          </div>}
          ...
          <div className="d-flex justify-content-between mt-2">
            ...
            <span className="fw-medium">₹{Number(OrderDetail?.taxJson?.reduce((acc, cur) => acc + Number(cur.value), 0)).toFixed(2)}</span>
          </div>
          ...
          <div className="d-flex justify-content-between fw-semibold fs-6">
            <span className='text-muted'>{OrderDetail?.paidOnDelivery > 0 ? "Pay On Delivery" : "Paid By Wallet"}</span>
            <span>₹{Number(OrderDetail?.orderTotal).toFixed(2)}</span>
          </div>
